...
description: >-
  ...
  and component patterns for the Grand Dental clinic website, inspired by ViScan Diagnostics.
---

# Grand Dental Clinic — Design System & UI Skill Guide

This skill guide defines the core design language, typography hierarchy, color tokens, and signature UI component patterns for the **Grand Dental Clinic** web platform, modeled after the modern clinical aesthetic of **ViScan Diagnostics**.

---

## 1. Design Philosophy & Mood

* **Core Mood:** High-Tech Clinical Precision meets Warm Modern Luxury.
* **Key Visual Attributes:**
  * **Approachable Medical Tech:** Friendly geometric typography with softened pastel bento cards instead of sterile, intimidating hospital whites.
  * **Categorical Clarity:** Structured section headers, pill badges, and clean visual hierarchy without redundant AI-style counter pills or flashing dots.
  * **Micro-Delight:** Smooth hover arrow shifts and subtle gradient text accents.
...
| :--- | :--- | :--- | :--- |
| `.type-display` | `text-3xl sm:text-5xl lg:text-6xl font-light tracking-tight leading-[1.1]` | 30px–60px / 300 / 1.1 | Hero section main $H_1$ titles (Preserved) |
| `.type-section-title` | `text-2xl sm:text-3xl lg:text-4xl font-medium tracking-tight leading-tight text-gray-900` | 24px–36px / 500 / 1.2 | Main section $H_2$ headings (Preserved) |
...
| `.type-eyebrow` | `text-xs sm:text-sm font-semibold uppercase tracking-wider text-[#74267A]` | 13px–14px / 600 / 1.4 | Section kicker labels & category tags |
...
| `.btn-card-link` | `text-sm font-semibold text-gray-900 hover:text-[#74267A]` | 14px / 600 | Interactive card action links |

---

## 3. Color Palette & Design Tokens

### Primary Brand Spectrum (Deep Plum & Vibrant Orchid)

```css
:root {
  --color-primary-900: #5C205E; /* Brand Dark Plum (Buttons, primary CTAs, deep accents) */
  --color-primary-800: #74267A; /* Deep Plum (Button hover, heading gradient end) */
  --color-primary-700: #922F9C; /* Medium Plum (Link highlights) */
  --color-primary-600: #B43ABF; /* Vibrant Magenta (Gradient text start) */
  --color-primary-500: #CE59DC; /* Bright Orchid (Subtle glows) */
  --color-primary-400: #D886E2; /* Light Orchid (Indicators & interactive dots) */
  --color-primary-100: #F7E6FA; /* Pale Lavender (Borders & badge backgrounds) */
  --color-primary-50:  #FCF5FE; /* Whisper Tint (Section backgrounds & soft cards) */
}
```

### Neutrals & Dark Surface Tokens
* **Dark AI / Technology Surface:** `#101828` (Rich Slate Black used for high-contrast diagnostic visualizer sections).
* **Text Primary ($H_1, H_2, H_3$):** `#111827` (`text-gray-900`).
* **Text Secondary (Body):** `#4B5563` (`text-gray-600`).
* **Text Muted (Captions & Footnotes):** `#6B7280` (`text-gray-500`).
* **Borders & Dividers:** `#E5E7EB` (`border-gray-200`) and `#F3F4F6` (`border-gray-100`).
* **Pure White:** `#FFFFFF`.

### Soft Pastel Card Tints
* **Doctor / Practitioner Bento:** `#F9EBFC`
* **Patient / Partner Bento:** `#EEEEFF`
* **App / Digital Feature Bento:** `#FDF0F2`
* **Diagnostic Metric Orbit:** `#F5F3F6`
...
* Background: `#101828`
* Radial Glow: `bg-[radial-gradient(circle,rgba(237,145,251,0.18),transparent_60%)]`
* Text Glow: `linear-gradient(251.6deg, #ED91FB 21.52%, #FFE1E1 50.16%, #FFD4D2 56.51%, #C76ECA 89.38%)`
...
#### Variant 1: Primary Dark Plum Pill (`.btn-pill-primary`)
```tsx
